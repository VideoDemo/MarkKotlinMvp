...
import com.mark.markkotlinmvp.durationFormat
import com.mark.markkotlinmvp.glide.GlideApp
import com.mark.markkotlinmvp.glide.GlideRoundTransform
import com.mark.markkotlinmvp.mvp.model.bean.HomeBean
import com.mark.markkotlinmvp.view.recyclerview.MultipleType
...
class VideoDetailAdapter(mContext: Context, data: ArrayList<HomeBean.Issue.Item>) :
    ...
    init {
        textTypeface = Typeface.createFromAsset(MarkApp.context.assets, "fonts/FZLanTingHeiS-L-GB-Regular.TTF")
    }
    ...
    private fun setVideoDetailInfo(data: HomeBean.Issue.Item, holder: ViewHolder) {
        ...
        with(holder) {
            getView<TextView>(R.id.tv_action_favorites).setOnClickListener {
                Toast.makeText(MarkApp.context, "喜欢", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_share).setOnClickListener {
                Toast.makeText(MarkApp.context, "分享", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_reply).setOnClickListener {
                Toast.makeText(MarkApp.context, "评论", Toast.LENGTH_SHORT).show()
            }
        }
    }
